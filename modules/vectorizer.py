...
def process_ai_response(raw_text: str) -> List[float]:
    ...
    match = re.search(r"```(\w+)?\s+(.*?)```", raw_text, re.DOTALL | re.IGNORECASE)
    ...
    logic_keywords = len(re.findall(r"\b(if|for|while|sub|def|class|dim|set|else|elif|then)\b", code_content, re.I))
    ...
    io_operations = len(re.findall(r"\b(open|save|connect|workbook|path|os\.|dir|file)\b", code_content, re.I))
    ...
    feature_vector = [float(loc), float(logic_keywords), float(io_operations), float(desc_len)]
